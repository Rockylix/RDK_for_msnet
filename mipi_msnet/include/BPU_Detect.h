...
public:
    BPU_detect(hbPackedDNNHandle_t packed_dnn_handle, 
        ...
        int type_of_pre);
    BPU_detect(hbPackedDNNHandle_t packed_dnn_handle, 
        const std::string& model_path); //FOR MIPI
    ~BPU_detect();
// private:
    bool GetModelInfo();
    bool LoadModel();
    ...
    void Mipi_PostProcess();
    void SaveResult(cv::Mat& disp);

    const int& Get_input_w() const{return _input_w;}
    const int& Get_input_h() const{return _input_h;}
    const int& Get_input_align_size() const { return _input_properties[0].alignedByteSize; }
    const hbSysMem& Get_input_sys_mem() const { return _input_sys_mem; }
