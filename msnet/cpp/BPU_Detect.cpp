...
bool BPU_detect::Preprocess(const cv::Mat& input_img1, const cv::Mat& input_img2)
{
    ...
    int img_data_size = target_h * target_w * 3; // 221184
    uint8_t *base_vir_addr = (uint8_t*)_input_sys_mem.virAddr;
    uintptr_t base_phy_addr = _input_sys_mem.phyAddr;
    ...
    // 更新 Tensor 结构体的地址和大小
    for(int i = 0; i < 2; i++) {
        _input_tensors[i].sysMem[0].virAddr = base_vir_addr + i * _input_properties[i].alignedByteSize;
        _input_tensors[i].sysMem[0].phyAddr = base_phy_addr + i * _input_properties[i].alignedByteSize;
